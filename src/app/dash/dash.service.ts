...
export class DashService {
  ...
  fetchKeys(name: string) {
    ...
    return this.http.get(this.keysReqUrl).pipe(
      map(responceData => {
        const keysArray = [];
  ...
  getValue(name: string, key: string) {
    var getValUrl = "dataStore/" + name + "/" + key;

    return this.http.get(getValUrl);
  }
  /*
  getValueObject(name: string, key: string){
    var getValurl = 'dataStore/'+name+'/'+key;
    return this.http.get(getValurl).pipe(map(
      val => {
        let data = []
